...
class TestMoneySpendService(TestCase):
    ...
    TANYA_ROW = DbResultRow(
        name="Tanya", base_cost=1500, custom_cost=None, build_limit=True
    )
    ...
    def test_retrieve_cost_custom(self) -> None:
        ...
        data_with_custom = DbResultRow(
            name="Harrier", base_cost=1200, custom_cost=960, build_limit=False
        )
    ...
    @patch(
        "app.service.money_spend_service.MoneySpendService.fetch_cached_data_or_else"
    )
    @patch("app.service.money_spend_service.MoneySpendService.spend_money")
    def test_spend_money_by_type(
        self, mock_spend_money: Mock, mock_fetch_data: Mock
    ) -> None:
    ...
    def test_spend_money(self) -> None:
        ...
        result = self.money_spend_service.spend_money(
            [self.BLACK_EAGLE_ROW, self.TANYA_ROW], 5000
        )
